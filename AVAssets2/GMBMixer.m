...
)
{
	...
	int bytesRead = 0;											  //Number of bytes of audio data read so far in this particular call to the callback
	//	int bytesToCopy = ((userData->totalBytesInBuffer - userData->bytePos) < (inNumberFrames * inNumChannels * sizeof(GMBAudioSample32BitFloat_t))) ?
	//									  userData->totalBytesInBuffer - userData->bytePos : (inNumberFrames * inNumChannels * sizeof(GMBAudioSample32BitFloat_t));
...
//	sampleBufferDelegate = [[GMBSampleBufferDelegate alloc] initWithSize:2048 * sizeof(Float32)];
...
{
//	CMSampleBufferRef bufferRef = [source copyNextSampleBuffer];
//	[sampleBufferDelegate readDataFromCMSampleBufferRef:bufferRef];
}
